...
\begin{document}
  ...
  \cite{geron2019hands}. Activation functions like Rectified Linear Unit $ReLU(z) = max(0, z)$ and hyperbolic tangent
  function $tanh(z) = 2\sigma(2z)-1$ are used to add nonlinearity, which theoretically gives a large enough deep neural
  network the ability to approximate any continuos function \cite{geron2019hands}.

  \subsubsection{Convultional Neural Networks (CNNs)}
