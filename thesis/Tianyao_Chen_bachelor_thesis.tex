...
a 50-layer Residual Network (ResNet-50) backbone on a very small dataset of only 50 images. And we still managed to
...
$mAP^{small}$ for $area < 32^2$, $mAP^{medium}$ for $32^2 < area < 96^2$, and $mAP^{big}$ for $area > 96^2$
\cite{liu2020deep, cocometrics}
...
As previously mentioned in section \ref{sec:cvForUnderwater}, there is limited research on deep learning for underwater
...
\cite{konovalov2019underwater} respectively used Fast R-CNN, a model similar to R-CNN, a sliding window with a CNN,
YOLOv3, and Xception.
...
For planktons and corals, we only found research for classification but not object detection
\cite{qin2015underwater, moniruzzaman2017deep}.

For amphora detection, we only found the study from Pasquet et al. \cite{mccarthy20193d, pasquet2017amphora} that used
a sliding window with a CNN on a high-resolution orthophoto (i.e. aerial photo).
...
SSD due to the following reasons:

\begin{itemize}
  \item Faster R-CNN is too computationally intensive and too slow for AUVs and UUVs.
  \item YOLOv5, the state-of-the-art model of the YOLO family, is under active development, and the authors have yet to
  publish a paper.
\end{itemize}
...
to one, eight, and seven respectively in the configuration file, while the rest of the fields were unchanged. The network
...
\begin{itemize}
  \item Our dataset is very limited. As previously mentioned in section \ref{sec:dlvscv}, the COCO dataset has 328
  ...
  As previously mentioned in section \ref{sec:cvForUnderwater}, the same challenges also apply to amphora
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{The validation images. Sources:
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{The test images. Sources: \cite{ionian, sanisera}.}
  \label{fig:test}
\end{figure}
...
very small and low-resolution dataset, and we still managed to achieve a 0.238 $mAP_{coco}$ and a 0.5392 $mAP@0.5$. Detecting
